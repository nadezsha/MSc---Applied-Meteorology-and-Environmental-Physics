...
dni_int = np.array([np.trapezoid(row, x=wl) if np.isfinite(row).any() else np.nan for row in vals])
df["DNI_integrated_Wm2"] = dni_int
...
# ---- Filter valid + SZA <= 90 ----
valid = np.isfinite(sza) & np.isfinite(dni_int) & (sza <= 90)
sza = sza[valid]
dni_int = dni_int[valid]

# ---- SAVE integrated table ----
out_table = in_path.with_name("sza_calculation_with_integrated_dni.csv")
...
plt.scatter(sza, dni_int, s=10, alpha=0.5, label="Samples")
plt.xlabel("Solar Zenith Angle (degrees)")
plt.ylabel("Integrated DNI (W/m²)")
plt.title("Integrated DNI vs Solar Zenith Angle (Raw Scatter)")
plt.grid(True, alpha=0.3)
plt.legend()
plt.tight_layout()

out_plot1 = fig_dir / "integrated_dni_vs_sza_scatter.png"
...
tmp = pd.DataFrame({"sza": sza, "dni": dni_int})
...
plt.xlabel("Solar Zenith Angle (degrees)")
plt.ylabel("Integrated DNI (W/m²)")
plt.title("Integrated DNI vs Solar Zenith Angle (Binned Line)")
plt.grid(True, alpha=0.3)
plt.legend()
plt.tight_layout()

out_plot2 = fig_dir / "integrated_dni_vs_sza_line.png"
