...
def main():
    ...
    for filename in os.listdir(INPUT_FOLDER):
        ...
        try:
            ...
            # 🔹 NEW: Skip if all wavelength columns are empty
            wl_cols = [c for c in reshaped.columns if c not in ("Datetime", "Exposure (msec)")]
            if reshaped[wl_cols].isna().all().all():
                print(f"[SKIP EMPTY] {filename}: all wavelength values are empty")
                continue
