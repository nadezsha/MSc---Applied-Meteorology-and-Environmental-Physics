...
% 
% subplot(2,3,5)
% plot(tstamp(1:end-1), WDIR(1:end-1), '-','LineWidth', 3);
% hold on;
% plot(tstamp(end), WDIR(end), '-','LineWidth', 3); % Plot the last point without connecting it
% hold off;
% xlabel('Hour of the day');ylabel('WDIR (deg)')
% set(gca,'XTick',0:3:24,'XGrid','on','FontSize',14);xlim([0 24])
% 
% 
...
% ylabel('Rain Type');
% title(['Rain Types Throughout the Day - ', dates{1}]);
% grid on;
% set(gca,'XTick',0:3:24,'XGrid','on','FontSize',14);xlim([0 24])
