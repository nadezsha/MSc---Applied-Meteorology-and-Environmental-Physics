# Postprocess Epanomi AERONET uvspec OUT files.
#
# For each file like:
#   epanomi_2025-04-22_sza_083p0.out
# it:
#   - parses date and sza
...
OUT_CSV = OUT_DIR / "epanomi_aeronet_integrated_by_sza.csv"

# Expected column names in OUT files (wavelength + 3 fields)
COLS = [
    "wavelength",
...
FNAME_RE = re.compile(
    r"[A-Za-z]+_(\d{4}-\d{2}-\d{2})_sza_([0-9]{1,3})p([0-9])",
    re.IGNORECASE,
)
...
def robust_read(fp: Path, skiprows: int = 9) -> pd.DataFrame:
    ...
    - Splits on whitespace and keeps only the first 4 numeric tokens.
    """
    rows = []
