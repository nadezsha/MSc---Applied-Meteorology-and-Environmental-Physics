...
FILE = r"C:\MSc - Applied Meteorology and Environmental Physics\MSc Thesis\AERONET\epanomi\DNI\integrated_data\epanomi_aeronet_integrated_by_sza.csv"
...
out_path = out_dir / (Path(FILE).stem + "_ecc_corrected_epanomi.csv")
df.to_csv(out_path, index=False)
print(f"Saved: {out_path}")

figures_dir = SCRIPT_DIR / "figures_epanomi"
...
plt.title("Irradiances corrected for Earth–Sun distance (Epanomi AERONET)")
plt.legend()
plt.grid()
plt.tight_layout()
plt.savefig(figures_dir / "epanomi_corrected_irradiances.png", dpi=300)
plt.show()
...
plt.xlabel("Solar Zenith Angle (deg)" if col_sza else "Index")
plt.ylabel("Irradiance (W/m²)")
plt.title("DNI corrected for Earth–Sun distance (Epanomi AERONET)")
plt.grid()
plt.tight_layout()
plt.savefig(figures_dir / "epanomi_corrected_DNI_irradiance.png", dpi=300)
plt.show()
...
plt.xlabel("Solar Zenith Angle (deg)" if col_sza else "Index")
plt.ylabel("Irradiance (W/m²)")
plt.title("DHI corrected for Earth–Sun distance (Epanomi AERONET)")
plt.grid()
plt.tight_layout()
plt.savefig(figures_dir / "epanomi_corrected_DHI_irradiance.png", dpi=300)
plt.show()
