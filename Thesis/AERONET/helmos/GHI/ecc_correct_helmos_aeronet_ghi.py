...
FILE = SCRIPT_DIR / "integrated_data" / "helmos_GHI_aeronet_integrated_by_sza.csv"
...
plt.title("Irradiances corrected for Earth–Sun distance (Helmos AERONET, inversion run)")
plt.legend()
plt.grid()
plt.tight_layout()
plt.savefig(figures_dir / "helmos_GHI_corrected_irradiances.png", dpi=300)
plt.show()
...
plt.xlabel("Solar Zenith Angle (deg)" if col_sza else "Index")
plt.ylabel("Irradiance (W/m²)")
plt.title("DNI corrected for Earth–Sun distance (Helmos AERONET, inversion run)")
plt.grid()
plt.tight_layout()
plt.savefig(figures_dir / "helmos_GHI_corrected_DNI_irradiance.png", dpi=300)
plt.show()
...
plt.xlabel("Solar Zenith Angle (deg)" if col_sza else "Index")
plt.ylabel("Irradiance (W/m²)")
plt.title("DHI corrected for Earth–Sun distance (Helmos AERONET, inversion run)")
plt.grid()
plt.tight_layout()
plt.savefig(figures_dir / "helmos_GHI_corrected_DHI_irradiance.png", dpi=300)
plt.show()