...
- Find the libRadtran OUT file for the same date with the closest SZA.
  Filenames are assumed like: helmos_YYYY-MM-DD_sza_083p0.out
...
# Save CSV inside: .../DNI/
SUMMARY_CSV = MODEL_OUT_DIR.parent / "dni_minSZA_daily_summary.csv"
...
def scan_model_files(model_dir: Path) -> List[Dict]:
    ...
    # 🔧 changed glob -> rglob so we also search subfolders like 2024-11-05
...
def main():
    ...
    for date_val, day_df in df_meas.groupby("date"):
        ...
        # Restrict interpolation to the union of [WL_MIN, WL_MAX]
        # (wl_meas is already in that range)
        edir_interp_mW = np.interp(wl_meas, wl_model, edir_model_mW)
        if SPECTRA_IN_MW:
            edir_interp_W = edir_interp_mW / 1000.0
        else:
            edir_interp_W = edir_interp_mW
        ...
        summary_rows.append({
            ...
            "eko_sza_deg": sza_meas,  # keep column name here if you want; or rename to 'eko_solar_zenith_deg'
