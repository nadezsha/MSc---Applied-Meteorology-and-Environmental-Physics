...
(AERONET–driven Helmos uvspec runs) with **measured spectral DNI**
...
2. Reads the **model spectral DNI** from libRadtran OUT files:
       helmos_YYYY-MM-DD_sza_XXXpX.out
   (Each OUT file corresponds to a specific date + SZA.)

3. For each day (6 days total):
...
This allows you to:
-------------------
• Evaluate spectral agreement between model and measurements  
• Identify biases due to aerosol absorption/scattering, gas absorption, etc.  
• Compare performance across days with different atmospheric conditions  
"""
...
# Base directory of libRadtran model OUT files (AERONET Helmos runs)
MODEL_BASE_DIR = Path(
...
def get_model_spectrum(date_str: str, sza_deg: float) -> pd.DataFrame | None:
    ...
    fname = f"helmos_{date_str}_sza_{sza_tag}.out"
...
for date_str in all_dates:
    ...
    # small summary so you see how close things are to 1
    print(
    ...
    # ratio is dimensionless → no units in label
    plt.ylabel("DNI_model / DNI_measured")
    plt.title(f"Spectral DNI Ratio (model/measurement) – {date_str}")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
