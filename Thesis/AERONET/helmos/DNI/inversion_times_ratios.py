...
AERONET_INV_DIR:
    ...
    filenames like YYYYMMDD_YYYYMMDD_Helmos_NTUA.lev15

Outputs
-------
For each date:
    helmos_YYYY-mm-dd_HHMM_dni_ratio_inversion_closest.png
        → model/measured spectral ratio for each inversion time

    helmos_YYYY-mm-dd_all_inversions_dni_ratio_inversion_closest.png
        → all inversion times plotted together

    helmos_YYYY-mm-dd_daily_mean_dni_ratio_inversion_closest.png
        → daily mean spectral ratio

Purpose
-------
This script makes it possible to directly compare:
    • AERONET-driven libRadtran modeled spectral DNI  
    • EKO MS-711 measured spectral DNI  
specifically *at the times of aerosol inversion retrievals*,  
capturing the most representative aerosol states of the atmosphere
and minimizing mismatches due to temporal or SZA differences.
"""
...
MEAS_FILE = Path(
    r"C:\MSc - Applied Meteorology and Environmental Physics\MSc Thesis\data\helmos\eko-ms711-dni-helmos\2024\sza_calculation\sza_calculation_with_integrated_dni.csv"
)
...
AERONET_INV_DIR = Path(
    r"C:\MSc - Applied Meteorology and Environmental Physics\MSc Thesis\AERONET\helmos\DNI\AERONET_inversions_data\all_data"
)

SITE_TAG = "Helmos_NTUA"
...
OUT_FIG_DIR = Path(
    r"C:\MSc - Applied Meteorology and Environmental Physics\MSc Thesis\AERONET\helmos\DNI\figures_inversion_times"
)
...
def build_model_map() -> Dict[pd.Timestamp, List[Tuple[float, Path]]]:
    ...
    Works for structures like:
      uvspec_runs/
        2024-11-05/
          helmos_2024-11-05_sza_015p0.out
          helmos_2024-11-05_sza_016p0.out
          ...
    """
    ...
    # recursively find all .out / .OUT files
    ...
    n_parsed = 0
    for f in files:
        stem = f.stem  # helmos_2024-11-05_sza_015p0
        m_date = date_re.search(stem)
        m_sza  = sza_re.search(stem)
        if not m_date or not m_sza:
            # show what we’re skipping so we can debug if needed
            print(f"  Debug: could not parse date/SZA from '{f.name}', skipping.")
            continue

        date_str = m_date.group(1)   # '2024-11-05'
        sza_tag  = m_sza.group(1)    # '015p0'
...
def read_aeronet_inversion_times(inv_file: Path) -> List[pd.Timestamp]:
    ...
    with open(inv_file, "r", encoding="utf-8") as f:
        for i, line in enumerate(f):
            # works for both AOD and SSA v3 files
    ...
    # read with this line as header
    df = pd.read_csv(inv_file, skiprows=header_line)

    # columns are like "Date(dd:mm:yyyy)" and "Time(hh:mm:ss)"
...
def build_aeronet_file_map() -> Dict[pd.Timestamp, Path]:
    ...
    Works with filenames like:
      20241105_20241105_Helmos_NTUA.lev15
      20241105_20241105_Helmos_NTUA.ssa
    """
    mapping: Dict[pd.Timestamp, Path] = {}

    # accept both .lev15 and .ssa, just in case
    for f in AERONET_INV_DIR.glob(f"*_{SITE_TAG}.*"):
        if f.suffix.lower() not in [".lev15", ".ssa"]:
            continue
        try:
            # first 8 chars = YYYYMMDD (in your example both dates are the same)
            ymd = f.name[:8]
...
def main():
    ...
    df_meas["Datetime"] = df_meas["Datetime"].dt.tz_localize(None)  # tz-naive
    ...
    df_meas[wl_cols] = df_meas[wl_cols] / 1000.0  # mW -> W
    ...
    for date_val, df_day in df_meas.groupby("date_ts"):
        ...
        # pick closest EKO row for each inversion time
        used_indices = []
        for t in inv_times:
        ...
        for idx, row in df_sel.iterrows():
            ...
            plt.title(f"Helmos {date_str} {dt.strftime('%H:%M')} (closest inversion + model SZA)")
            plt.grid(True)

            fig_name = OUT_FIG_DIR / f"helmos_{date_str}_{time_str}_dni_ratio_inversion_closest.png"
            plt.tight_layout()
        ...
        if all_ratio_curves:
            ...
            for label, curve in zip(all_labels, all_ratio_curves):
                plt.plot(wl_nm, curve)   # semi-transparent so overlaps are visible
            plt.axhline(1.0, linestyle="--")
            plt.xlabel("Wavelength [nm]")
            plt.ylabel("DNI_model / DNI_meas [-]")
            plt.title(f"Helmos {date_str} – all inversion times (closest EKO + model SZA)")
            plt.grid(True)

            # Optional legend: only if not too many curves
            if len(all_labels) <= 15:
                plt.legend(all_labels, fontsize=7, ncol=2)

            overlay_name = OUT_FIG_DIR / f"helmos_{date_str}_all_inversions_dni_ratio_inversion_closest.png"
            plt.tight_layout()
            ...
            # ---------- existing daily mean figure (unchanged) ----------
            ...
            plt.title(f"Helmos {date_str} mean over inversion times (closest EKO + model SZA)")
            plt.grid(True)

            daily_fig_name = OUT_FIG_DIR / f"helmos_{date_str}_daily_mean_dni_ratio_inversion_closest.png"
            plt.tight_layout()
